...
let gen_shuffle_fn p lmap rmap bindings fn_name =
  ...
  let lkey_types_unwrap = wrap_tmaybes @: key_map_types_for p lmap in
  let lkey_types = wrap_ttuple lkey_types_unwrap in
  ...
  mk_global_fn fn_name
    ...
    [result_types] @: (* return *)
    mk_destruct_tuple "l_key" lkey_types_unwrap id_l @: (* destruct the key *)
