...
let declare_foreign_functions p =
  ...
  let add_delta_foreign p map_id = mk_foreign_fn
    ...
    (wrap_ttuple [t_vid; wrap_tlist @: wrap_ttuple @: map_types_for p map_id])
    (canonical TUnit)
  in
  let map_related =
...
let trig_stmt_k3_list = 
  List.fold_left 
    ...
    (mk_empty @: wrap_tlist @: wrap_ttuple [t_trig_id; t_stmt_id])
    trigs_stmts_with_rhs_map
in
(* Note: what happens when we send the data here? Does it include vids in the
 * tuples like the data in pushes?
 *)
...
let filter_corrective_list = mk_global_fn filter_corrective_list_name
  ...
  (mk_sort (* sort so that early vids are first for performance *)
    (mk_flatten @: mk_map
      (mk_lambda (wrap_args ["trig_id", t_trig_id; "stmt_id", t_stmt_id])
        (mk_filtermap
