...
module type S =
  sig
    ...
    val filter_gt: elt -> t -> t
    val filter_geq: elt -> t -> t
    val filter_lt: elt -> t -> t
    val filter_leq: elt -> t -> t
...
module Make(Ord: OrderedType) =
  struct
    ...
    *)
    ...
    let filter_comp comp x m =
      let rec loop = function
        ...
        | Node(l, v, r, _) ->
            ...
            if op c 0 then
              join (if do_l then loop l else l) v (if do_r then loop r else r)
            else
              loop other
      in loop m
    ...
    let to_list m = fold (fun k acc -> k::acc) m []
