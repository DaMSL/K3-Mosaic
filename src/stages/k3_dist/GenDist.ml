...
let send_do_trig_bitmap =
  let init = mk_map (mk_lambda' unknown_arg mk_cfalse) @@ mk_var D.my_peers.id in
  create_ds ~init "send_do_trig_bitmap" @@ wrap_tvector t_bool

let sw_send_do_trig_name_of_t t = "sw_send_do_trig_"^t
let sw_send_do_trig_fn c t =
  let args = args_of_t_as_vars_with_v c t in
  let rcv_do_trig_nm = nd_rcv_do_trig_name_of_t t in

  mk_global_fn (sw_send_do_trig_name_of_t t)
    (["batch_id", t_int]@args_of_t_with_v c t) [] @@
  mk_block @@
  [mk_set_all send_do_trig_bitmap.id [mk_cfalse]] @
  (List.flatten @@ List.map (fun s ->
    (* first route to lmap *)
    let lmap = P.lhs_map_of_stmt c.p s in
    let rmaps = P.rhs_maps_of_stmt c.p s in
    let lroute_key, lroute_pat_idx = P.key_pat_from_bound c.p c.route_indices s lmap in
    [
      R.route_lookup c lmap
      (mk_cint lmap::lroute_key)
      (mk_cint lroute_pat_idx) @@
      mk_iter_bitmap'
        (mk_insert_at send_do_trig_bitmap.id (mk_var "ip") [mk_ctrue])
        R.route_bitmap.id
    ] @
...
let nd_rcv_do_trig c s_rhs_lhs s_rhs t =
  let send_puts =
    ...
    * specific IP *)
      ...
      [
        ...
        (* XXX Change *)
          (mk_block [
            mk_if (mk_at' send_put_bitmap.id (mk_var D.me_int.id))
              (mk_apply' rcv_put_nm args)
              mk_cunit
          ])
      ]
  in
  ...
  let send_fetches_of_rhs_maps  =
    let args = args_of_t_as_vars_with_v c t in
    let rcv_fetch_nm = rcv_fetch_name_of_t t in
      ...
      (List.map
        (fun (stmt_id, rhs_map_id) ->
          ...
          R.route_lookup c rhs_map_id
            ...
            (mk_cint pat_idx) @@
          mk_let ["do_send_fetch"] (mk_at' R.route_bitmap.id @@ mk_var D.me_int.id) @@
            mk_if (mk_var "do_send_fetch")
              (mk_block [
                ...
                mk_apply' rcv_fetch_nm (args@[mk_cint stmt_id; mk_cint rhs_map_id])
              ])
              mk_cunit)
        s_rhs)
  in
...
let nd_rcv_fetch_trig c trig =
  ...
  mk_global_fn fn_name
    (D.nd_rcv_fetch_args c trig @ ["stmt_id", t_stmt_id; "map_id", t_map_id])
    [] @@ mk_block [
...
let nd_dispatcher_buf_inner =
  let e = ["sender_ip", t_int; "batch_id", t_vid; "poly_queue", poly_queue.t] in
...
let nd_trig_dispatcher_trig c =
  ...
  mk_let_block ["sender_ip"; "batch_id"; "poly_queue"] (mk_var "batch_data")
  [
    ...
    mk_if (mk_eq (mk_var "num") (mk_var nd_dispatcher_next_num.id))
      ...
      mk_insert nd_dispatcher_buf.id
        [mk_var "num"; mk_tuple [mk_var "sender_ip"; mk_var "batch_id"; mk_var "poly_queue"]]
    ;
                        ...
                        mk_poly_at_with' nm @@
                          mk_lambda' id_ts @@
                            mk_if (mk_var "do_insert")
                              (mk_apply' (sw_send_do_trig_name_of_t @@ "insert_"^nm) @@
                                ids_to_vars @@ "batch_id"::"vid":: (fst_many @@ tl id_ts)) @@
                              if c.gen_deletes then
                               mk_apply' (sw_send_do_trig_name_of_t @@ "delete_"^nm) @@
                                ids_to_vars @@ "batch_id"::"vid":: (fst_many @@ tl id_ts)
                              else mk_cunit)
                      acc_code)
