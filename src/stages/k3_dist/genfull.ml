...
on_insert_T_switch(QUERY_1_1_pR1_pS1T_T__C, QUERY_1_1_pR1_pS1T_D) {
         ...
         .~ (List.fold_left
           (fun acc_code ((stmt_id, rhs_map_id))) ->
             let route_fn = route_for rhs_map_id in
              let key, pat = key_and_pat_from_bound stmt_id rhs_map_id trig_args
             .<combine(
   ...
   .~ (List.fold_left
        (fun acc_code (stmt_id, lhs_map_id, complete_trig_addr) -> 
          let route_fn = route_for lhs_map_id in
    ...
    * IP *)
   iter(\(ip, stmt_id_cnt_list) ->
          send(promote_address(T_Put,ip), bound_with_v, stmt_id_cnt_list)
     groupby(
      ...
      .~fetch_map_trigger_name(C::D::vid) {
        iter(\(ip, tuples) ->
             send(promote_address(push_map_trigger_name, ip),
                 bound, tuples, vid),
             .~shuffle_fn(key, pat, 
                 foreign(slice(rhs_map_name, .~key_and_pat_from_bound(stmt_id,
                     rhs_map_name, C::D))),
                 false)
       }
       .~acc_code>.
